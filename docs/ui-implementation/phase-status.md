...
- Shared infrastructure is normalized under singular `src/lib/`: shadcn/Tailwind utilities remain in `src/lib/utils.ts`, while the shared query client lives in `src/lib/query-client.ts`.
