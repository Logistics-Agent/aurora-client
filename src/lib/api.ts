  ...
  private registerInterceptors() {
    ...
    this.axiosInstance.interceptors.response.use(
      ...
      async (error: AxiosError) => {
        ...
        const requestUrl = normalizeUrl(requestConfig?.url ?? "");
        const isAuthRequest = requestUrl.startsWith("api/v1/auth/");

        if (status === 401 && requestConfig && !requestConfig._retry && !isAuthRequest) {
