...
import { LiveMailWorkspace } from "../live-mail-workspace";
...
export function MailWorkspace(props: MailWorkspaceProps): React.JSX.Element {
  if (props.repository) {
    return <MockMailWorkspace {...props} repository={props.repository} />;
  }

  return (
    <MailAccessState user={props.user} isLoading={false}>
      <LiveMailWorkspace {...props} />
    </MailAccessState>
  );
}

function MockMailWorkspace({
  user,
  resourceScope,
  initialThreadId,
  repository,
}: MailWorkspaceProps & { repository: MailMockRepository }): React.JSX.Element {
  const [routeThreadId, setRouteThreadId] = useState(initialThreadId);
  const [mobilePane, setMobilePane] = useState<MobilePane>(initialThreadId ? "thread" : "list");
  ...
  const layoutClass =
    viewportMode === "desktop"
      ...
      : viewportMode === "mid"
        ...
        : "grid-cols-1";
  const layoutName =
    viewportMode === "desktop" ? "three-pane" : viewportMode === "mid" ? "two-pane" : "single-pane";
      ...
      >
        <header className="flex flex-wrap items-center justify-between gap-3 border-b border-border pb-3">
          <div>
            ...
            >
              UI-only demo; not connected to live mail.
            </p>
          </div>
          {viewportMode === "mid" ? (
            <button
              type="button"
              className="rounded-lg border border-border px-3 py-2 text-sm font-medium transition-colors outline-none focus-visible:ring-2 focus-visible:ring-ring motion-reduce:transition-none"
        ...
        {viewportMode === "mobile" && mobilePane === "queue" ? (
          <button
            type="button"
            className="w-fit rounded-lg border border-border px-3 py-2 text-sm font-medium transition-colors outline-none focus-visible:ring-2 focus-visible:ring-ring motion-reduce:transition-none"
