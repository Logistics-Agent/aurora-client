# PHASE 13 — 3D INTEGRATION

## Objective

Harden reusable 3D primitives and network→region→shipment transitions.
...
Review Command Center, Live Map, Route Planning, Tracking, Shipment route context. Extract only genuinely reusable 3D primitives to `components/three`. Harden GLB loading, camera transitions, route arcs, markers, shipment motion, lazy loading, DPR/performance and fallback states.
